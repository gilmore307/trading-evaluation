...
class ReplayDatasetPreparationTests(unittest.TestCase):
    def test_prepare_replay_dataset_writes_candidate_policy_replay_bundle(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(prepared.manifest["candidate_fold_id"], "fold_2016-01_2017-06")
            ...
            self.assertEqual(prepared.manifest["pre_replay_target_refs"], ["AAPL"])
            self.assertEqual(
                prepared.manifest["source_storage_policy"]["duplicate_source_body_policy"],
                "forbidden_for_reusable_source_partitions",
            )
            self.assertEqual(
                prepared.manifest["source_storage_policy"]["coverage_scan_role"],
                "read_existing_canonical_source_partitions",
            )
            self.assertEqual(
                prepared.manifest["source_storage_policy"]["provider_gap_fill_role"],
                "write_missing_canonical_source_partitions_only",
            )
