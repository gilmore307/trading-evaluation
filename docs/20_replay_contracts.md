...
- equity/options replay allocation is ranked best-first with no fixed top-N position-count limit by default. The `position_notional_fraction` setting is the minimum new-target allocation floor, not a single-position notional cap. For listed options, replay buys one contract when a single contract costs more than the floor, and otherwise rounds contract count up so planned notional is at least the floor while cash remains available.
...
- C05 replay sizing records planned option quantity from the selected contract cost and the minimum allocation floor; C06 may verify but must not alter that quantity.
