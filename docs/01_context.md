...
| `trading-evaluation` | Independently evaluates candidates against frozen benchmark contracts, emits fold settlement / promotion eligibility evidence, and owns model activation records. |
| `trading-manager` | Schedules fold work and records workflow state; it consumes evaluation status but does not own benchmark judgment or activation. |
...
- Activation records publish active model config refs only; execution remains a separate consumer and broker/account boundary.
