...
- equity/options replay allocation is ranked best-first inside a finite portfolio-capacity contract. The default equity/options sleeve has five simultaneous risk slots, derived from the default `0.20` target-allocation fraction. M04 must emit the target allocation as a fraction of total portfolio/account budget. Under the current fixed-slot policy, an orderable entry's model fraction must meet the minimum actionable slot fraction unless the model explicitly declares a partial-slot or optimizer-weighted allocation mode; risk/confidence compression belongs in eligibility, ranking, and rejection, not in silent undersized orders. Replay converts the accepted model fraction to a target notional floor; the default target-allocation fraction is only a fallback when the model field is missing. For listed options, replay buys one contract when a single contract costs more than the target floor, and otherwise rounds contract count up so planned notional is at least that floor while cash and position-capacity remain available. Research runs may request an explicit wider or unbounded position-count override, but such receipts are not promotion-compatible.
...
- The replay portfolio must keep scanning after all five risk slots or most cash are committed. A new candidate may replace the weakest held position only after point-in-time option expression is executable, target allocation is compatible with the slot contract, and the candidate clears the score-scale-aware switch threshold. The current default switch threshold is `0.00001` on `diagnostic_rank_score`, recorded as `switch_threshold_policy = score_scale_aware_absolute_rank_delta`; the old fixed `0.05` threshold is not promotion-compatible because it is outside the observed rank-score scale.
