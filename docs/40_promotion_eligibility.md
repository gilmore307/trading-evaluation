...
- Eligibility must be based on a complete fold-stack run: Layer 1 through Layer 9 model evaluation for the same fold must be complete before promotion review starts.
- Eligibility applies to one pinned Layer 1-9 version bundle. The bundle is accepted or rejected as a whole; layer-local results support diagnostics only.
