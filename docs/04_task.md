...
- Replay coverage is now evaluated against explicit `training_target_ref`, `tradable_universe_ref`, `tradable_target_refs`, and fold scope.
...
- The replay contract validator and CLI support fold-bound candidate-policy replay windows with explicit `candidate_fold_id`, `training_target_ref`, and `tradable_universe_ref`.
...
- Manager replay dispatch blocks any frozen dataset whose `training_target_ref` does not match the completed training fold.
- Manager evaluation blocks any replay receipt whose `training_target_ref` does not match the completed training fold.
